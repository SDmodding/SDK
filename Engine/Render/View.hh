...
namespace Render
{
	UFG_INLINE Illusion::CTarget* GetBackBufferTarget()
	{
	...
	{
		Illusion::CTarget* pBackBuffer = GetBackBufferTarget();
		return { static_cast<float>(pBackBuffer->mWidth), static_cast<float>(pBackBuffer->mHeight) };
	}
